...
const AIChess: React.FC<GameProps> = ({ onGameOver, difficulty, isSeniorMode }) => {
  // Configuración de tiempo según dificultad
  // Fácil: 5 min, Normal: 3 min, Difícil: 1.5 min
  // Senior: 10 minutos (Básicamente sin prisa)
    ...
    if (status === 'player' && timeLeft > 0) {
      timer = setInterval(() => {
        setTimeLeft((prev) => {
          if (prev <= 1) {
            setStatus('timeout');
            setMessage('¡Tiempo agotado! Gana la IA.');
            onGameOver(50); // Puntos mínimos por participación
            return 0;
          }
          return prev - 1;
        });
    ...
    if (game.isGameOver()) {
      if (game.isCheckmate()) {
        ...
        if (isPlayerWin) {
           ...
           setMessage('¡Jaque Mate! Gana la IA.');
           setTimeout(() => onGameOver(100), 3000);
        }
      } else {
        setStatus('draw');
        setMessage('Tablas / Empate.');
      ...
      for (let colIndex = 0; colIndex < 8; colIndex++) {
        ...
        squares.push(
          <div
            ...
            className={`
              relative flex items-center justify-center text-3xl sm:text-4xl select-none cursor-pointer transition-colors
              ${isDark ? 'bg-slate-600' : 'bg-slate-300'}
              ${isSelected ? '!bg-teal-500/60' : ''}
              ${isValidMove ? '!bg-yellow-400/50' : ''}
            `}
          ...
          >
            ...
            {isValidMove && piece && (
              <div className="absolute inset-0 border-4 border-rose-500/50 rounded-none"></div>
            )}
            ...
            {piece && (
              <span 
                className={`
                  ...
                  ${piece.color === 'w' ? 'text-white drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]' : 'text-slate-900 drop-shadow-[0_1px_1px_rgba(255,255,255,0.5)]'}
                `}
              >
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-4 w-full max-w-lg mx-auto">
      <div className="flex justify-between w-full items-center gap-4">
        <div className={`
          flex items-center gap-2 px-4 py-2 rounded-xl border-2 font-mono font-bold text-xl
          ${timeLeft < 30 ? 'bg-rose-500/20 border-rose-500 text-rose-500 animate-pulse' : 'bg-slate-800 border-slate-700 text-slate-200'}
        `}>
          <span>⏱️</span>
      ...
      <div className="flex items-center justify-between w-full">
         ...
         <button 
          onClick={handleReset}
          className="px-4 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-white text-xs font-bold uppercase transition-colors"
        >
          Reiniciar
        </button>
      </div>
