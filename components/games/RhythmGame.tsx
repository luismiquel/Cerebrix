...
const RhythmGame: React.FC<GameProps> = ({ onGameOver, isSeniorMode, difficulty, isDailyChallenge, currentRound = 1 }) => {
  ...
  const gameLoop = useCallback((time: number) => {
    ...
    if (difficulty === 'master') rate = 280; // Frecuencia extrema
    ...
    setNotes(prev => {
      ...
      if (difficulty === 'master') finalSpeed = 2.4; // Caída vertiginosa
  ...
  const handleAction = (lane: number) => {
    ...
    setNotes(prev => {
      // Ajustar ventanas de impacto según dificultad
      const hitThreshold = difficulty === 'master' ? 10 : 18;
      const perfectThreshold = difficulty === 'master' ? 3.5 : 6;
      ...
      if (targetIdx !== -1) {
        ...
        // Penalización en modo Maestro
        if (difficulty === 'master') {
            triggerVibrate([50, 20]);
    ...
    <div className="flex flex-col items-center h-full w-full max-w-lg mx-auto overflow-hidden relative select-none touch-none">
      <div className="flex justify-between w-full px-6 mb-4 z-20">
        ...
        <div className="flex flex-col items-center">
            {difficulty === 'master' && (
                <span className="bg-indigo-600 text-white text-[8px] px-2 py-0.5 rounded-full font-black animate-pulse tracking-tighter">MAESTRO x2.5</span>
            )}
        </div>
      ...
      <div className={`relative flex-1 w-full bg-slate-950/60 rounded-[3rem] border-4 overflow-hidden shadow-inner transition-colors duration-500 ${difficulty === 'master' ? 'border-indigo-500/30 shadow-indigo-500/10' : 'border-slate-800'}`}>
        {LANES.map(l => (
            <div key={l} className={`absolute h-full w-[25%] transition-opacity duration-100 ${laneFlash[l] ? 'opacity-50' : 'opacity-0'}`} style={{ left: `${l*25}%`, background: `linear-gradient(to top, ${LANE_COLORS[l]}, transparent)` }} />
        ))}
        ...
        {notes.map(n => !n.hit && (
          <div 
            ...
            style={{ 
                ...
                boxShadow: `0 0 ${difficulty === 'master' ? '35' : '25'}px ${LANE_COLORS[n.lane]}`,
      ...
      <div className="grid grid-cols-4 gap-3 w-full h-36 mt-6 px-2">
        {LANES.map(l => (
          <button 
            key={l} 
            onPointerDown={(e) => { e.preventDefault(); handleAction(l); }} 
            className={`h-full rounded-[2.5rem] bg-slate-800 border-b-[10px] active:translate-y-2 active:border-b-0 transition-all flex items-center justify-center shadow-2xl ${difficulty === 'master' ? 'border-indigo-950 active:bg-indigo-900/40' : 'border-slate-950'} ${laneFlash[l] ? 'bg-slate-700' : ''}`}
          >
